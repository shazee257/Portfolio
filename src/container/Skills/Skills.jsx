...
const Skills = () => {
  ...
  useEffect(() => {
    ...
    client.fetch(skillsQuery).then((data) => {
      // const res = data.sort((a, b) => a._id - b._id);
      setSkills(data);
      // console.log(res);
    });
  }, []);
      ...
      <div className="app__skills-container">
        ...
        <div className="app__skills-exp">
          {experiences.map((experience) => (
            <motion.div className="app__skills-exp-item" key={experience.year} style={{ height: '40px' }}>
              <div className="app__skills-exp-year">
                <p style={{ width: '150px', fontSize: '13px' }} className="bold-text">{experience.year}</p>
              </div>
                ...
                {experience.works.map((work) => (
                  <div key={work.name}>
                    <motion.div whileInView={{ opacity: [0, 1] }} transition={{ duration: 0.5 }} className="app__skills-exp-work" data-tip data-for={work.name}>
                      ...
                      <p style={{ width: '200px', fontSize: '13px' }} className="p-text">{work.company}</p>
                    </motion.div>
